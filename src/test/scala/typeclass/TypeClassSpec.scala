...
import typeclass.Boot.SuperVipCollections4s.syntax.GetSizeScoreOps
import typeclass.Boot.SuperVipCollections4s.{GetSizeScore, MutableBoundedCache, PackedMultiMap, SizeScore}

import scala.typeclass.ImplicitsHomeworkSpec.TestValue
...
class TypeClassSpec extends AnyFreeSpec with Matchers {
  ...
  "Iterate2 instances provided" - {
    "for Map" in {
      assertCompiles(
        """import typeclass.Boot.SuperVipCollections4s._
          |import typeclass.Boot.SuperVipCollections4s.instances._
          |
  ...
  "TwitCache" - {
    import typeclass.Boot.MyTwitter._
